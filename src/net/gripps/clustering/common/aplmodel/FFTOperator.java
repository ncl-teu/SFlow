 ...
 *
 */
public class FFTOperator extends AplOperator{
     ...
     * Singleton Object
     */
    private static FFTOperator singleton;

    /**
     *
     * @return
     */
    public static FFTOperator getInstance(){
        if(FFTOperator.singleton == null){
            FFTOperator.singleton = new FFTOperator();

        }

        return FFTOperator.singleton;
    }
    ...
    public void constructTask(String filename) {
        try {
            ...
            id.add(new Long(1));
            apl.setIDVector(id);
            FFTOperator.getInstance().setApl(apl);
            ...
            for(int i=1 ; i<= this.layer;i++){
                for(int j = 1; j <= this.scale; j++){
                    ...
                    FFTOperator.getInstance().getApl().addTask(updatedTask);
    ...
    public BBTask assignDependencyProcess() {
        ...
        //1レイヤ目のタスク要素数を見る。
        BBTask upperTask = FFTOperator.getInstance().getApl();
       ...
       // AbstractTask end = upperTask.getTaskList().getLast();
        ...
        while(taskIte.hasNext()){
            ...
            if (this.isIDEqual(task.getIDVector(),
                    FFTOperator.getInstance().getApl().getEndTask())) {
                continue;
            }
        }

        AbstractTask endTask = FFTOperator.getInstance().getApl().findTaskByLastID(new Long(tasknum));
        ...
        while(dpredIte.hasNext()){
            ...
            AbstractTask dpredTask = FFTOperator.getInstance().getApl().findTaskByLastID(dd.getFromID().get(1));
            this.updateAncestor(ansSet, dpredTask);
        }
        //全体のPGの命令数を集計する．
        BBTask task = (BBTask) FFTOperator.getInstance().calculateInstructions(FFTOperator.getInstance().getApl());
        FFTOperator.getInstance().setApl(task);

...
public class HTMLpageOperator {
     ...
     * Singleton Object
     */
    private static HTMLpageOperator singleton;

    /**
     *
     * @return
     */
    public static HTMLpageOperator getInstance(){
        if(HTMLpageOperator.singleton == null){
            HTMLpageOperator.singleton = new HTMLpageOperator();
        }

        return HTMLpageOperator.singleton;
    }

    private HTMLpageOperator() {
    ...
    public void constructTask(String filename) {
        try {
            ...
            this.startTaskSize = Integer.valueOf(prop.getProperty("task.html.starttasksize")).intValue();
            this.endTaskSize = Integer.valueOf(prop.getProperty("task.html.endtasksize")).intValue();
            this.midTaskSize_max =  Integer.valueOf(prop.getProperty("task.html.midtasksize.max")).intValue();
            this.midTaskSize_min =  Integer.valueOf(prop.getProperty("task.html.midtasksize.min")).intValue();
            this.datasize_max =  Integer.valueOf(prop.getProperty("task.html.datasize_max")).intValue();

            this.datasize_min =  Integer.valueOf(prop.getProperty("task.html.datasize_min")).intValue();
            this.midsize = Integer.valueOf(prop.getProperty("task.html.midtasknum")).intValue();
           ...
           // this.startup = startupTimes * this.comunit;
            ...
            id.add(new Long(1));
            apl.setIDVector(id);
            HTMLpageOperator.getInstance().setApl(apl);
		...
		if(i == 0) {
			...
			AbstractTask startTask = this.buildChildTask(true,weight);
              		HTMLpageOperator.getInstance().getApl().addTask(startTask);
		...
		if(i == 1) {
			//int midsize = 100;
			//１００個のタスクを追加。weightは1-100までの乱数
			for(int j = 0; j < this.midsize; j++) {
			//AbstractTask midTask = this.buildChildTask(true,(long)(Math.random() * 100) + 10);
            AbstractTask midTask = this.buildChildTask(true, (long)this.generateLongValue(this.midTaskSize_min, this.midTaskSize_max));
            HTMLpageOperator.getInstance().getApl().addTask(midTask);
			}
		}
		...
		if(i == 2) {
			...
			AbstractTask endTask = this.buildChildTask(true,weight);
              		HTMLpageOperator.getInstance().getApl().addTask(endTask);
		}
	...
	//関数CALLの場合，子タスクは外部との依存はない．ただし，STARTとENDは違うけど．
        ...
        //1レイヤ目のタスク要素数を見る。
        BBTask upperTask = HTMLpageOperator.getInstance().getApl();
       ...
       // AbstractTask end = upperTask.getTaskList().getLast();
        ...
        if (end.getType() == Constants.TYPE_CONDITION) {
            //HTMLpageOperator.getInstance().findTask(end.getIDVector()).setType(Constants.TYPE_BASIC_BLOCK);
	 ...
	 AbstractTask startTask = upperTask.findTaskByLastID(new Long(1));
        for(int i=2;i<this.midsize;i++){
            AbstractTask midTask = upperTask.findTaskByLastID(new Long(i));
           // long dataSize = (int)(Math.random() * 100) + 10; //1-100
            long dataSize = (long)this.generateLongValue(this.datasize_min, this.datasize_max);
            DataDependence dd = new DataDependence(startTask.getIDVector(), midTask.getIDVector(), 0, 0, 0);
	...
	//中央タスク-エンドタスク間のDD定義
	AbstractTask endTask = upperTask.findTaskByLastID(new Long(midsize+2));
        for(int i=2;i<this.midsize;i++){
            //Start <->中間タスクの依存関係の定義
            AbstractTask midTask = upperTask.findTaskByLastID(new Long(i));
            long dataSize = 1; 
            DataDependence dd = new DataDependence(midTask.getIDVector(),endTask.getIDVector(), 0, 0, 0);
	    //引数は(矢印元タスクID,矢印先タスクID,0,0,0)
            //long datasize = this.generateLongValue(this.ddedge_sizemin, this.ddedge_sizemax);
            dd.setMaxDataSize(dataSize);
            dd.setAveDataSize(dataSize);
    ...
    public CustomIDSet updateAncestor(CustomIDSet allSet, AbstractTask task){
        ...
        while(dpredIte.hasNext()){
            ...
            AbstractTask dpredTask = HTMLpageOperator.getInstance().getApl().findTaskByLastID(dd.getFromID().get(1));
            //まずは先行タスクのIDを先祖に追加する．

 ...
 * Date: 2007/07/17
 */
public class Main_AplCreate_Gauss2 {
    public static void main(String[] args) {
        ...
        try {
            ...
            for (int i = 0; i < LOOP_NUM; i++) {
                ...
                machineNum = Integer.valueOf(prop.getProperty("task.gj.machinenum")).intValue();

                using = Integer.valueOf(prop.getProperty("algorithm.clustering.using")).intValue();
                ...
                if(dagtype == 1){
                 //タスクグラフを構築する
                    ...
                    startup = GaussianOperator.getInstance().getStartup();
                }else{
                ...
                //SIRT-δのマシン数:
                BBTask dag = null;
                int machineNum_cass = 0;
                int machineNum_dsc = 0;

                switch(using){
                    case 2 :
                        dag = (BBTask)aplList.get(0);
                        PostClusteringManager pMgr_CASS = new PostClusteringManager(aplList.get(0), filename, 0, machineNum);
                        BBTask retApl_cass = pMgr_CASS.postClusteringLB(machineNum);
                        aplList.add(retApl_cass);
                        machineNum_cass= dag.getTaskClusterList().size();
                        break;
                    case 4:
                                        //DSC
                        dag = (BBTask)aplList.get(0);
                        PostClusteringManager pMgr_DSC = new PostClusteringManager(aplList.get(0), filename, 0, machineNum);
                        BBTask retApl_dsc = pMgr_DSC.postClusteringWP2(machineNum);
                        aplList.add(retApl_dsc);
                        machineNum_dsc = dag.getTaskClusterList().size();
                        break;
                    default :

                        //LB
                        /*
                        BBTask lbDAG =(BBTask)LBaplCopy;
          ...
          //      aplList.add(DSCApl);
                ...
                double CCR = (double)(apl_first.getEdgeWeight()-(startup*sum_edgenum) / (double)apl_first.getTaskWeight());
                ...
                while (aplIte2.hasNext()) {
                   ...
                   // double mkRate = (double) apl2.getMakeSpan() / (double) apl2.getCpLen();
                    ...
                    System.out.println("Speed-UP: " + Calc.getRoundedValue(value * apl2.getTaskClusterList().size()));

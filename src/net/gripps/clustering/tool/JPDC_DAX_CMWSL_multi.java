 ...
 */
public class JPDC_DAX_CMWSL_multi {
    public static void main(String[] args) {
        ...
        try {
            ...
            for (int i = 0; i < LOOP_NUM; i++) {
                ...
                int jobNum = Integer.valueOf(prop.getProperty("dax.multiple.num")).intValue();
                ...
                //以降は，シングルの場合の計算
                for (int jnum = 0; jnum < jobNum; jnum++) {
                    DAXOperator.getInstance().getMultiList();
                ...
                System.out.print(":LBCNS_Default:" + ret_group.getMakeSpan() + ":" + retSet.getList().size() + ":" + SLR_distance);


                Hashtable<Long, CPU> cpu_table_homo = new Hashtable<Long, CPU>();
                CMWSL_Algorithm hetero_homo = new CMWSL_Algorithm(apl_homo, filename, env_group_homo);

                //メイン処理を行う．
                BBTask ret_group_homo0 = hetero_homo.process();
                MWSL_RCPScheduling mwsl_rcp_homo = new MWSL_RCPScheduling(ret_group_homo0, filename, env_group_homo);
                BBTask ret_group_homo = mwsl_rcp_homo.process();
                ...
                CMWSL_Algorithm hetero_com = new CMWSL_Algorithm(apl_com, filename, env_group_com);

                //メイン処理を行う．
                BBTask ret_group_com0 = hetero_com.process();
                MWSL_RCPScheduling mwsl_rcp_com = new MWSL_RCPScheduling(ret_group_com0, filename, env_group_com);
                BBTask ret_group_com = mwsl_rcp_com.process();
                ...
                while (jobIte.hasMoreElements()) {
                    ...
                    CMWSL_Algorithm allnodes = new CMWSL_Algorithm(job_allnodes, filename, envall_single);
                    BBTask ret_allnodes = allnodes.process();
                    MWSL_RCPScheduling allnodes_rcp = new MWSL_RCPScheduling(ret_allnodes, filename, envall_single);
                    BBTask ret_allnodes2 = allnodes_rcp.process();
                    long mkSpan_allnodes = ret_allnodes2.getMakeSpan();
                    ...
                    CMWSL_Algorithm lbcns_default = new CMWSL_Algorithm(job_default, filename, env_group_single);
                    BBTask ret_default = lbcns_default.process();
                    MWSL_RCPScheduling default_rcp = new MWSL_RCPScheduling(ret_default, filename, env_group_single);
                    BBTask ret_defaul2 = default_rcp.process();
                    long mkSpan_default = ret_defaul2.getMakeSpan();
                    ...
                    CMWSL_Algorithm lbcns_similar = new CMWSL_Algorithm(job_similar, filename, env_group_homo_single);
                    BBTask ret_similar = lbcns_similar.process();
                    MWSL_RCPScheduling similar_rcp = new MWSL_RCPScheduling(ret_similar, filename, env_group_homo_single);
                    BBTask ret_similar2 = similar_rcp.process();
                    long mkSpan_similar = ret_similar2.getMakeSpan();
                    ...
                    CMWSL_Algorithm lbcns_com = new CMWSL_Algorithm(job_com, filename, env_group_com_single);
                    BBTask ret_com = lbcns_com.process();
                    MWSL_RCPScheduling com_rcp = new MWSL_RCPScheduling(ret_com, filename, env_group_com_single);
                    BBTask ret_com2 = com_rcp.process();
                    long mkSpan_com = ret_com2.getMakeSpan();

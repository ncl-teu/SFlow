...
/**
 * クラスタサイズの下限値を様々な値に変動させるためのメインクラスです．
 * 2010/07/06
 */
public class Main_AplCreate_Delta {
        ...
        try {
            ...
            for (int i = 0; i < LOOP_NUM; i++) {
               ...
               // System.out.print(apl_first.getTaskWeight() + ":" + outDegree_Max + ":" + edgeSize_Max + ":" + apl_first.getEdgeNum() + ":" + apl_first.getEdgeWeight() + ":");
                ...
                CustomIDSet t_set = new CustomIDSet();
                for(i=0; i< len; i++){
                    ...
                    if(i==0){
                       ...
                       t_set = lbc.getMaxTaskSetInPath();
                       ret.setAnsPathSet(t_set);
                        apl_first.setAnsPathSet(t_set);
                    }else{
                        ret.setAnsPathSet(t_set);

                    }

                    //CustomIDSet pathSet = lbc.getMaxTaskNumInPath();
                    aplList.add(ret);
                }
                ...
                while (aplIte2.hasNext()) {
                    ...
                    //System.out.print("Num: " +machineNum+": "+ret_mkRate + ": "+dag.getLinearNum() +": " + dag.getNonLinearNum() +"/");
                    Iterator<Long> tIte = apl2.getAnsPathSet().iterator();
                    CustomIDSet cSet = new CustomIDSet();
                    while(tIte.hasNext()){
                        Long tID = tIte.next();
                        AbstractTask task = apl2.findTaskByLastID(tID);
                        cSet.add(task.getClusterID());
                    }
                    //クラスタに対するループ
                    long linear = 0;
                    long nonlinear = 0;
                    Iterator<Long> cIte = cSet.iterator();
                    while(cIte.hasNext()){
                        Long cID = cIte.next();
                        TaskCluster cls = apl2.findTaskCluster(cID);
                        if(cls.isLinear()){
                            linear++;
                        }else{
                            nonlinear++;
                    ...
                    System.out.print(machineNum+":"+linear/5+":" + nonlinear/5 + ":");

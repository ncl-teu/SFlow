...
/*
        ...
        VNF[] vArray = (VNF[])queue.toArray();
        double totalIdle = 0.0;
        for(int i=0;i<vArray.length;i++){
            //i番目タスクを取得
            VNF v = vArray[i];
            //start時刻
            double start = v.getStartTime();
            double end = v.getFinishTime();
            if(i < vArray.length-1){
                totalIdle += vArray[i+1].getStartTime() - end;
            }
...
*/
        ...
        while(dpredIte.hasNext()){
            DataDependence dpred = dpredIte.next();
            if(maxPredData <= dpred.getMaxDataSize()){
                maxPredData = dpred.getMaxDataSize();
            }
        }
        ...
        while(dsucIte.hasNext()){
            ...
            if(maxSucData <= dsuc.getMaxDataSize()){
                maxSucData = dsuc.getMaxDataSize();
            }
        }
        double g = this.calcComTime(maxPredData, this.getAveBW()) +
                this.calcExecTime(vnf.getWorkLoad(), this.getAveSpeed()) +
                this.calcComTime(maxSucData, this.getAveSpeed());



        //queueから，アイドル時間の合計を計算する方法
        PriorityQueue<VNF> queue = vcpu.getVnfQueue();
     ...
     * @return
     */
    protected double calcESTWitoutDL(VNF vnf, VCPU cpu) {
        double arrival_time = 0;
        double dl_finish_time = 0;
     ...
     * @return
     */
    public double calcImageComTime(double dataSize, VCPU vcpu) {
        //DCの情報@vcpu側
